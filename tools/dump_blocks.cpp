...
int main(int argc, char** argv){
    if(argc<2){ std::cerr<<"usage: dump_blocks file\n"; return 1; }
    ...
    for(int b=0;b<100 && pos+4<buf.size();++b){
        ...
        for(size_t num_pos = pos+1; num_pos+1 < buf.size() && num_pos < pos+4096; ++num_pos){
            ...
            size_t offsets_section_start = (num_pos >= static_cast<size_t>(num)*2) ? (num_pos - static_cast<size_t>(num)*2) : SIZE_MAX;
            if(offsets_section_start==SIZE_MAX) continue;
            ...
            if(offsets_section_start >= pos && offsets_section_start + num*2 == num_pos){
                ...
                if(num>0){
                  ...
                  if(first_off < (num_pos - pos)) { found = num_pos; break; }
                } else { found = num_pos; break; }
            }
        }
        ...
        for(size_t i=0;i<num && i<10;i++){
            uint16_t off; std::memcpy(&off, buf.data()+offsets_section_start + i*2, 2);
            std::cout<<"  off["<<i<<"]="<<off<<"\n";
        }
